...
function loadConfig(){config=readConfig();$('cfgMax').value=config.max;$('cfgPawn1').value=config.pawn1;$('cfgPawn2').value=config.pawn2;config.offers.forEach((v,i)=>{if($(`cfgOffer${i+1}`))$(`cfgOffer${i+1}`).value=v});$('pawnMinRate').value=config.pawn1;$('pawnMaxRate').value=config.pawn2;applyPriceLimit()}
...
$('priceRange').addEventListener('input',e=>{state.price=Math.min(n(e.target.value),n(config.max)||defaults.max);$('referencePrice').textContent=`${state.price.toFixed(2)} €/g`;calculate()});
