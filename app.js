...
function loadConfig(){$('cfgMax').value=config.max;$('cfgPawn1').value=config.pawn1;$('cfgPawn2').value=config.pawn2;config.offers.forEach((v,i)=>{if($(`cfgOffer${i+1}`))$(`cfgOffer${i+1}`).value=v});$('pawnMinRate').value=config.pawn1;$('pawnMaxRate').value=config.pawn2;syncReferenceLimit()}
function renderHistory(){const list=$('historyList');let items=[];try{items=JSON.parse(localStorage.getItem('goldHistory')||'[]')}catch{}list.innerHTML=items.map(x=>`<div class="history-item"><strong>${eur(x.value)}</strong><div>${Number(x.weight).toFixed(2)} g · ${Number(x.eq).toFixed(2)} g eq. 18K</div><small>${x.date}</small></div>`).join('')}
...
$('priceRange').addEventListener('input',e=>{const max=Math.max(1,n(config.max)||defaults.max);state.price=Math.min(Math.max(n(e.target.value),1),max);e.target.value=String(state.price);$('referencePrice').textContent=`${state.price.toFixed(2)} €/g`;calculate()});
